...
# vim
bindkey -v
# Wait for multi-character key sequences.
KEYTIMEOUT=10
...
## ---fzf-tab---
source <(fzf --zsh)
# Let fzf-tab own Tab while retaining fzf's Ctrl-T, Alt-C, and Ctrl-R widgets.
...
gh_downloader() {
  ...
  if (( $# != 3 )); then
    print -u2 'Usage: gh_downloader <owner/repo> <asset-regex> <filename>'
    return 2
  fi
  ...
  local pattern=$2
  local filename=$3
  local download_url tmp exit_code
  ...
  download_url=$(
    curl --fail --location \
      ...
      jq -er --arg pattern "$pattern" '
        [.assets[] | select(.name | test($pattern)) | .browser_download_url] as $urls |
        if ($urls | length) == 1 then
          $urls[0]
        elif ($urls | length) == 0 then
          error("no release asset matches pattern: \($pattern)")
        else
          error("multiple release assets match pattern: \($pattern)")
        end
      '
  ) || return 1

  tmp=$(mktemp -- "${filename}.part.XXXXXX") || return 1

  curl --fail --location --output "$tmp" "$download_url"
  exit_code=$?
