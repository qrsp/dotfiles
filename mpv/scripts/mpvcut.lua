...
local options = {
	...
	avif_crf = 50,							-- the crf value to use for animated .avif clips, lower numbers mean higher quality
  avif_cpu_used = 8,
...
ACTIONS.ENCODE_GIF = function(d)
	...
	if options.animated_encoding_type == "avif" then
		...
		table.insert(args, "libaom-av1")
		table.insert(args, "-crf")
		table.insert(args, tostring(options.avif_crf or 50))
		table.insert(args, "-cpu-used")
		table.insert(args, tostring(options.avif_cpu_used or 8))
